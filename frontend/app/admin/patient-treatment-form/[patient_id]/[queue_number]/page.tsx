...
import { Plus, Trash, Search, XCircle, AlertTriangle, Check   } from "lucide-react";
...
export default function TreatmentForm() {
  ...
  const [prescriptions, setPrescriptions] = useState<Prescription[]>([
    { medication: "", dosage: "", frequency: "", quantity: "", start_date: "", end_date: "" },
  ]);
  ...
  const searchMedicines = async (query: string) => {
    ...
    try {
      const token = localStorage.getItem('access');
      const url = `http://127.0.0.1:8000/medicine/medicine-search/?q=${encodeURIComponent(query)}`;
                    ...
                    {activePrescriptionIndex === index && (
                      <div className="absolute z-30 mt-2 w-[300px] rounded-lg border bg-white shadow-xl">
                          ...
                          <div className="flex items-center justify-center p-4 text-gray-500">
                            <svg className="h-5 w-5 animate-spin mr-2 text-blue-600" fill="none" viewBox="0 0 24 24">
                          ...
                          <div className="max-h-60 overflow-y-auto">
                            {(medicineResults || []).map((medicine, idx) => (
                              <button
                                key={idx}
                                type="button"
                                className="w-full text-left p-3 hover:bg-gray-50 focus:bg-gray-100 transition-colors border-b last:border-b-0 focus:outline-none"
                                onClick={() => {
                                  console.log("Selected medicine id:", medicine.id);
                                  handlePrescriptionChange(index, "medicine_id", medicine.id?.toString() || "");
